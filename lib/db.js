...
export const restaurants = {
  ...
  async getBySlug(slug) {
    // Ruta PÚBLICA (anónima): NO traer columnas sensibles. La carta la ve
    // cualquiera con la anon key, así que jamás deben salir de acá los secretos
    // de cobro/mensajería (mp_access_token, mp_refresh_token, mp_user_id,
    // mp_preapproval_id, twilio_*) ni owner_id/producer_id. Se listan solo las
    // columnas públicas. mp_public_key SÍ se incluye (es la clave pública que el
    // checkout de MercadoPago necesita). El panel del dueño y el maestro usan
    // rutas autenticadas (getByOwner/getById) que conservan acceso total.
    const { data, error } = await supabase
      .from('restaurants')
      .select(`
        id, slug, name, description, tagline, logo_url, cover_url, primary_color,
        address, city, country, currency, business_type, is_active, is_open,
        can_take_orders, opening_hours, instagram_handle, facebook_url, website_url,
        phone, email, timezone, plan, plan_status, mp_connected, mp_public_key,
        subscription_expires_at, subscription_status, subscription_tier,
        tano_messages_limit, tano_messages_used, tano_month, tano_month_count,
        tano_reset_at, trial_end, trial_ends_at, wa_followup_config, wa_status,
        wants_whatsapp, delivery_channels_config, created_at, updated_at,
        menu_categories(
...
// orders
// ---------------------------------------------------------------------------
// Columnas de orders que puede leer el rol anónimo (comensal / cocina demo).
// EXCLUYE customer_phone y payment_id: son datos privados del cliente que no
// deben salir por la anon key. La cocina REAL (staff autenticado) sí lee todo
// para notificar por WhatsApp; por eso las lecturas de staff no usan estas cols.
const ORDER_PUBLIC_COLS = 'id, restaurant_id, session_id, table_number, status, total, notes, channel, customer_name, created_at, updated_at, paid_at'

export const orders = {
  /** Staff: pedidos activos de un restaurante.
   *  opts.publico = true → lectura anónima (cocina demo): columnas públicas, sin teléfono. */
  async listActive(restaurantId, opts = {}) {
    const sel = opts.publico
      ? `${ORDER_PUBLIC_COLS}, order_items(*)`
  ...
  async create(payload) {
    // .select() explícito (sin customer_phone): el comensal anónimo puede INSERT
    // el teléfono pero NO leerlo de vuelta (policy de columnas). Pedir '*' daría 42501.
    const { data, error } = await supabase
      .from('orders')
      .insert(payload)
      .select(ORDER_PUBLIC_COLS)
      .single()
    if (error) {

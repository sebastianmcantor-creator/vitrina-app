...
export function getPlanKey(restaurant) {
  ...
  // Trial de 14 días: destraba el plan que SE VENDE del rubro. Sin esto, el
  // negocio nuevo quedaba con límites Free (sin pedidos ni cocina) durante la
  // "prueba gratuita" y no podía probar justamente lo que se le vende (bug QA).
  // Restaurante → 'rest-menu' (Solo Menú: pedidos+cocina+pagos, todo lo que
  // necesita). Antes devolvía 'rest-combo' → el panel mostraba "Combo" (plan
  // que ni ofrecemos) y parecía "ya contratado".
  if (isTrialing(restaurant)) {
    return restaurant?.business_type === 'restaurant' ? 'rest-menu' : 'local-mkt'
  }
  return 'free'
}

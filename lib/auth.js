...
// ---------------------------------------------------------------------------
// Login por email (enlace mágico) — para dueños que no usan Google.
// Envía un link al email; al tocarlo entran sin contraseña. Si el email no
// tenía cuenta, se crea automáticamente (shouldCreateUser).
// ---------------------------------------------------------------------------
export async function signInWithEmail(email) {
  const { error } = await supabase.auth.signInWithOtp({
    ...
    options: {
      ...
      emailRedirectTo: `${window.location.origin}/panel.html`,
    },
  })
  if (error) throw new Error(error.message)
}
